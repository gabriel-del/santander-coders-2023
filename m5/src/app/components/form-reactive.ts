...
@Component({
  ...
  <form class="border p-3 mb-3" [formGroup]="formTask" (ngSubmit)="submitTask()">
    <div class="form-group">
      ...
      <div class="text-danger" *ngIf="formTask.get('title')?.hasError('required')" > Titulo é obrigatório </div>
    </div>
    <div class="form-group">
      <label for="description">Descrição</label>
      <input class="form-control" id="description" type="text" name="description" formControlName="description" />
      <div class="text-danger" *ngIf="formTask.get('description')?.hasError('required')" > Descrição é obrigatório </div>
    </div>
    <div class="form-group">
